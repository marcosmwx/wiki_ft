...
Defaults  secure_path="/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin:/snap/bin" #Essa configuração impede que um atacante adicione comandos maliciosos em diretórios customizados no PATH do usuário para serem executados com sudo.
